...
<div v-for="program in multiVideos.content" :key="program.id"  class="card">
    <div class="row ">
        <div class="col-md-4">
...
<div class="col-md-12">
<div v-if="multiVideos.content!=0"  class="col-md-12">
    <div class="clearfix">
        <div class="hint-text">Showing <b><span v-if="multiVideos.number!=(multiVideos.totalPages-1)">{{((page.pageNumber+1)*multiVideos.numberOfElements)}}</span>
            <span v-else>{{((multiVideos.number*multiVideos.size)+multiVideos.numberOfElements)}}</span></b> out of <b>{{ multiVideos.totalElements }}</b> entries</div>
        ...
        <ul class="pagination">
            ...
            <span> {{page.pageNumber+1}} Out of {{multiVideos.totalPages}}</span>
            <button v-if="page.pageNumber!=(multiVideos.totalPages-1)" @click="nextPage()" style="margin:10px;" class="btn btn-primary">Next</button>
        </ul>
    </div>
</div> 
</div>      
...
<script src="./js/multiVideo.js"></script>
